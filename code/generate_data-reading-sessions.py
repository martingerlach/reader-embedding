...
- returns filename where reading sessions are stored locally
    - ../output/sessions/reading-sessions-actors_<WIKI>_<START>_<STOP>
    
- USAGE:
PYSPARK_PYTHON=python3.7 PYSPARK_DRIVER_PYTHON=python3.7 spark2-submit --master yarn --executor-memory 8G --executor-cores 4 --driver-memory 2G  generate_data-reading-sessions.py -l simple
- optional
    - t1, start-date (incusive) YYYY-MM-DD (inclusive)
    - t2, end-date (exclusive) YYYY-MM-DD (exclusive)
'''
...
def main():
    ...
    parser.add_argument("--start","-t1",
                        default=None,
                        type = str,
                        help="start day to parse [inclusive] (YYYY-MM-DD-HH); default: previous day - 7days")

    parser.add_argument("--end","-t2",
                        default=None,
                        type = str,
                        help="end day to parse [exclusive] (YYYY-MM-DD-HH); default: current day")
    ...
    (requests
     ...
     .map(filter_unique_articles) ## remove repeated occurrence of same article in session
     .filter(lambda x: len(x)>=nlen_min) ## only sessions with at least length nlen_min
...
def filter_unique_articles(requests):
    """
    pageviews that are requested multiple times (e.g. a-b-a) 
    are handled inconsistently across user-agents. in some cases 
    the webrequest only keeps a-b. thus we only keep the first occurrence of a pageview.
    furthermore, in many cases a.2 (the second "a") simply copies 
    some information from a.1 (the first "a") such as the referer. 
    this will naturally include the filtering of consecutive articles.
    """
    r = requests[0]
    t = r['page_id']
